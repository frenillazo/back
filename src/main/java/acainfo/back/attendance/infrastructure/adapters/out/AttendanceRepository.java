...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    /**
     * Get students with low attendance in a group (below threshold)
     */
    @Query("SELECT a.studentId, " +
           "CAST(COUNT(CASE WHEN a.status IN ('PRESENTE', 'TARDANZA') THEN 1 END) AS double) * 100.0 / " +
           "CAST(COUNT(a) AS double) as attendanceRate " +
           "FROM Attendance a " +
           "WHERE a.session.subjectGroup.id = :groupId " +
           "GROUP BY a.studentId " +
           "HAVING attendanceRate < :threshold")
    List<Object[]> findStudentsWithLowAttendance(
        @Param("groupId") Long groupId,
        @Param("threshold") Double threshold
    );

    /**
     * Get attendance summary for a student across all groups
     */
    @Query("SELECT a.session.subjectGroup.id, COUNT(a), " +
           "CAST(COUNT(CASE WHEN a.status IN ('PRESENTE', 'TARDANZA') THEN 1 END) AS double) * 100.0 / " +
           "CAST(COUNT(a) AS double) " +
           "FROM Attendance a " +
           "WHERE a.studentId = :studentId " +
           "GROUP BY a.session.subjectGroup.id")
    List<Object[]> getAttendanceSummaryByGroupForStudent(@Param("studentId") Long studentId);

    /**
     * Find sessions with perfect attendance (all students present)
     */
    @Query("SELECT a.session.id " +
           "FROM Attendance a " +
           "GROUP BY a.session.id " +
           "HAVING COUNT(CASE WHEN a.status NOT IN ('PRESENTE', 'TARDANZA') THEN 1 END) = 0")
    List<Long> findSessionsWithPerfectAttendance();

    /**
     * Find sessions with low attendance (below threshold percentage)
     */
    @Query("SELECT a.session.id, " +
           "CAST(COUNT(CASE WHEN a.status IN ('PRESENTE', 'TARDANZA') THEN 1 END) AS double) * 100.0 / " +
           "CAST(COUNT(a) AS double) as attendanceRate " +
           ...
           "GROUP BY a.session.id " +
           "HAVING attendanceRate < :threshold")
    List<Object[]> findSessionsWithLowAttendance(@Param("threshold") Double threshold);

    /**
     * Count sessions where student was present in a date range
     */
    @Query("SELECT COUNT(a) FROM Attendance a " +
           ...
           "AND a.session.scheduledStart >= :startDate " +
           "AND a.session.scheduledStart <= :endDate")
    long countEffectiveAttendanceInDateRange(
        @Param("studentId") Long studentId,
        @Param("startDate") LocalDateTime startDate,
        @Param("endDate") LocalDateTime endDate
    );
    ...
    /**
     * Get recent attendance records for a student (last N days)
     */
    @Query("SELECT a FROM Attendance a WHERE a.studentId = :studentId " +
           "AND a.session.scheduledStart >= :since " +
           "ORDER BY a.session.scheduledStart DESC")
    List<Attendance> findRecentAttendanceForStudent(
        @Param("studentId") Long studentId,
    ...
    /**
     * Find all attendance records recorded by a specific teacher/user
     */
    @Query("SELECT a FROM Attendance a WHERE a.recordedById = :userId " +
           "ORDER BY a.recordedAt DESC")
    List<Attendance> findByRecordedById(@Param("userId") Long userId);

    /**
     * Find attendance records for sessions taught by a teacher
     */
    @Query("SELECT a FROM Attendance a WHERE a.session.subjectGroup.teacher.id = :teacherId " +
           "ORDER BY a.session.scheduledStart DESC")
    List<Attendance> findByTeacherId(@Param("teacherId") Long teacherId);

    /**
     * Find unjustified absences that can be justified
     */
    @Query("SELECT a FROM Attendance a WHERE a.status = 'AUSENTE' " +
           "ORDER BY a.session.scheduledStart DESC")
    List<Attendance> findUnjustifiedAbsences();

    /**
     * Find recently justified absences (last N days)
     */
    @Query("SELECT a FROM Attendance a WHERE a.status = 'JUSTIFICADO' " +
           "AND a.justifiedAt >= :since " +
           "ORDER BY a.justifiedAt DESC")
    List<Attendance> findRecentlyJustifiedAbsences(@Param("since") LocalDateTime since);
